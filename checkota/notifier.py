...
def build_notification_message(update: RegionUpdate) -> str:
    ...
    sdk = esc(str(update.sdk_message), quote=False) if update.sdk_message else None
    ...
    return (
        ...
        f"<b>Device:</b> {esc(str(update.cfg.model), quote=False)}{esc(region_line_raw, quote=False)}\n"
        f"\n"
        f"<b>Title:</b> {esc(str(update.title), quote=False)}\n"
        f"{os_line}\n"
        ...
        f"{update.desc!s}\n\n"
        f"<b>Size:</b> {esc(str(update.size), quote=False)}\n"
        + (
            f"<b>Incremental:</b> <code>{esc(str(inc), quote=False)}</code>\n"
            if inc
            else ""
        )
        + (f"<b>Security patch:</b> {esc(str(spl), quote=False)}\n" if spl else "")
        + f"<b>Fingerprint:</b> <code>{esc(str(update.target_fp), quote=False)}</code>"
        + (
            f"\n<b>Build date:</b> {esc(str(build_date), quote=False)} (CST)"
            if build_date
            else ""
        )
        + (
            f"\n<b>Google OTA link:</b> <code>{esc(update.url, quote=False)}</code>"
            if update.url
            else ""
        )
    )
