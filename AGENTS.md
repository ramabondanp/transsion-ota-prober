...
| `--reg` sweep failed every config lacking the region | `processor.py`, `cli.py` | A directory filter is a selection, not a demand: `load_config_regions()` returns `(0, [])` for a per-file miss when `is_sweep_mode(args)` (a single `-c` miss still errors), and `scan_region_filter()` pre-scans the sweep so an empty selection fails the run once -- before any work only when every config loaded cleanly, otherwise after the sweep so its load errors are reported too |
| One TAB or CR in the description degraded the whole notification to literal markup | `message_text.py` | `sanitize_html()` folds CRLF/lone CR to LF and tabs to space (`_normalize_kept_controls()`) before the control scan, matching `_safe_description()`, which deliberately keeps `\t` and `\r`; previously `is_safe_code_point()` rejected the whole message and `send()` fell back to escaped plain text, so Telegram rendered `&lt;b&gt;Device:&lt;/b&gt;` |
